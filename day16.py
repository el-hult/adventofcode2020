...
def part_two(mydata):
  ...
  for ticket in nearby_tickets:
    valid = True
    for value in ticket:
      if not any((low1 <= value <= high1) or (low2 <= value <= high2) for low1, high1, low2, high2 in rules.values()):
        valid = False
        break
    if valid:
  ...
  for field_num in range(n_fields):
    possible_positions = \
      [pos for pos in range(n_fields) if ok_matrix[field_num][pos] == 1 and pos not in field_order]    
    assert len(possible_positions) == 1, "This input needs a DFS style algorithm to allocate fields"
