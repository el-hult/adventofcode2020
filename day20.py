...
D4 = [SYM_ID, SYM_R90, SYM_R180, SYM_R270, SYM_FLIP_H, SYM_FLIP_V, SYM_FLIP_D1, SYM_FLIP_D2]
type Sym = int
type TileNum = int
...
type Allocations = list[list[OriTile | None]]  # 2D array of tiles, where each tile is an OriTile or None
...
tile_data_2 = {tile_num: 
               {ori: enact_symmetry(tile_data[tile_num], ori) for ori in range(8)} for tile_num in tile_nums}
pprint.pprint(tile_data_2)

# when allocating a tile to the array, we store tile number and the symmetry on it
array_allocations: list[list[OriTile | None]] = [[None for _ in range(array_size)] for _ in range(array_size)]
...
def hash_state(state: list[list[OriTile | None]]) -> str:
  ...
  flattened_state = tuple(x for row in state for x in row ) # TODO check if any Tiles have identical data (i.e.g maybe (1,SYM_ID) and (3,SYM_R90) look the same, then their hash should be the same), so that we can deduplicate hashes
  return hash(flattened_state)

revealed_states = {hash_state(array_allocations)}
frontier = [array_allocations] # the state stack to pop from when expanding the next state
...
# Verify the solution
assert all(x is not None for row in state for x in row), "Every position in the array should be filled"
used_tiles = set(x[0] for row in state for x in row)
assert len(used_tiles) == array_size * array_size, "All tiles should be used exactly once"
...
for r in range(array_size-1):
  for c in range(array_size-1):
    ...
    if not does_match(right_tile, this_tile, DIR_RIGHT):
      print(f"Tile {this_tile[0]} at ({r},{c}) does not match right tile {right_tile[0]} at ({r},{c+1})")
...
image_data = [
  ['.' for _ in range(array_size * 8)] for _ in range(array_size * 8)
]
for tile_r in  range(array_size):
  for tile_c in range(array_size):
    ...
    for r in range(8):
      for c in range(8):
        image_data[tile_r * 8 + r][tile_c * 8 + c] = '#' if tile_image[r + 1][c + 1] else '.'  # skip borders
