  ...
  for allergen in allergens:
    if allergen not in possible_ingredients_by_allergen:
      possible_ingredients_by_allergen[allergen] = (
        ingredients.copy()
      )  # without this copyy, some allergents share the same instance of the ingredients set. That is not right.
    else:
      possible_ingredients_by_allergen[allergen] &= (
        ingredients  # ampersand means intersection
      )
...
for food in raw.strip().splitlines():
  ...
  count += sum(
    1 for ing in ingredients if ing in ingredients_with_no_possible_allergens
  )
...
while any(len(v) == 1 for k, v in possible_ingredients_by_allergen.items()):
  to_allocate = next(
    (k for k, v in possible_ingredients_by_allergen.items() if len(v) == 1)
  )
...
assert len(possible_ingredients_by_allergen) == 0, (
  "There should be no allergens left to allocate"
)
