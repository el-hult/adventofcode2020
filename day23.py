...
def play_one_move(all_cups,current_cup):
  ...
  second = first.next
  third = second.next
  destination_cup_value = current_cup.value - 1 if current_cup.value > 1 else len(all_cups)
...
def initialize(raw:str, part_two: bool):
  ...
  cups = sorted(cups, key=lambda c: c.value) # sort by value, so that cup 1 is at position 0 in the list. for quick lookups!
...
assert ans_two < 999980000099, "I had a mistake in first try :( Just ran 1 million rounds instead of 10 million"
