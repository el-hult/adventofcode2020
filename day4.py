...
def parse_pass(passport:dict): 
    try:
        ...
        if not (1920<=passport['byr'] <= 2002): raise ValueError("bad byr")
        if not (2010<=passport['iyr'] <= 2020): raise ValueError("bad iyr")
        if not (2020<=passport['eyr'] <= 2030): raise ValueError("bad eyr")
        if ecl not in ["amb","blu","brn","gry","grn","hzl","oth"]: raise ValueError("bad ecl")
        if hgt:
            value = int(hgt.group(1))
            unit = hgt.group(2)
            passport['hgt'] = (value,unit)
            if (unit == "cm" and not 150 <= value <= 193) or(unit == "in" and not 59 <= value <= 76):
                raise ValueError('bad hgt')
        else:
            raise ValueError(f"invalid hgt")
        if hcl:
        ...
        #print(f"{passport} is invalid. {e}")
        return None

#
# Test data
#
assert sum(pass2dict(l) is not None for l in test_data1.split("\n\n")) == 2
assert all(parse_pass(pass2dict(l)) is None for l in test_data2.split("\n\n"))
assert all(parse_pass(pass2dict(l)) is not None for l in test_data3.split("\n\n"))



s1 = sum(pass2dict(l) is not None for l in data.split("\n\n"))
assert s1==233
print("Part A:",s1)

dicted_passes = [pass2dict(l) for l in data.split("\n\n")]
...
assert all( ecl in ['amb','blu','brn','gry','grn','hzl','oth'] for ecl in map(itemgetter('ecl'),valid_passes))
assert all( 
    (unit == 'cm' and 150 <= val <= 193) or
    (unit=='in' and 59 <= val <= 76)
