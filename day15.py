...
def part_one(input, to_report=2020):
  starting_numbers = tuple(map(int, input.split(",")))
  last_turn_this_was_spoken = [-1] * (
    to_report + 1
  )  # use a list to avoid dict overhead
...
def part_two(input):
  return part_one(input, to_report=30000000)


def main():
  true_input = read_input(15)
  ansA = part_one(true_input)
  assert ansA == 610

  ansB = part_two(true_input)
  assert ansB == 1407
